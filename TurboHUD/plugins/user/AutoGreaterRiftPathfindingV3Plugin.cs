namespace Turbo.Plugins.User
{
    ...
    public class AutoGreaterRiftPathfindingV3Plugin : BasePlugin, IInGameWorldPainter, IAfterCollectHandler
    {
        ...
        // 边界跟随算法相关属性
        private bool IsFollowingBoundary { get; set; }
        private Vector2 BoundaryFollowDirection { get; set; }
        private List<Vector2> BoundaryPoints { get; set; }
        private HashSet<Vector2> VisitedBoundaryPoints { get; set; }
        private int BoundaryFollowTimeout => 10000; // 边界跟随超时时间（毫秒）
        private IWatch BoundaryFollowTimer { get; set; }
        private float BoundaryDistanceThreshold => 10.0f; // 判定为边界的距离阈值
        private float BoundaryPointInterval => 5.0f; // 边界点记录间隔
        private int StuckCount { get; set; } // 卡住计数
        private Vector2 LastStuckPosition { get; set; } // 上次卡住位置
        private int MaxStuckCount => 3; // 最大卡住次数

        // 战斗位移控制相关属性
        private Vector2 CombatStartPosition { get; set; }
        private IWatch CombatPositionCheckTimer { get; set; }
        private int CombatPositionCheckInterval => 2000; // 战斗位置检查间隔（毫秒）
        private float MaxCombatDisplacement => 40.0f; // 最大战斗位移距离

        private List<string> DebugMessages = new List<string>();
        public IFont DebugFont { get; private set; }
        private string LogFilePath => Path.Combine(Directory.GetCurrentDirectory(), "TurboHUD", "logs", "AutoGreaterRiftPathfindingV3Plugin_Log.txt");
        
        public AutoGreaterRiftPathfindingV3Plugin()
        {
            Enabled = true;
            ShowPath = true;
            AutoNavigate = false;
            ...
            BoundaryFollowDirection = new Vector2(1, 0); // 默认沿着X轴正方向移动
            BoundaryPoints = new List<Vector2>();
            VisitedBoundaryPoints = new HashSet<Vector2>();
            StuckCount = 0;
            LastStuckPosition = default(Vector2);
            CombatStartPosition = default(Vector2);
        }
        ...
        public override void Load(IController hud)
        {
            ...
            // 初始化边界跟随计时器
            BoundaryFollowTimer = Hud.Time.CreateWatch();
            BoundaryFollowTimer.Start();
            // 初始化战斗位移检查计时器
            CombatPositionCheckTimer = Hud.Time.CreateWatch();
            CombatPositionCheckTimer.Start();
        ...
        public void PaintWorld(WorldLayer layer)
        {
            ...
            DebugFont.DrawText($"边界跟随: {(IsFollowingBoundary ? "开启" : "关闭")}", uiRect.Left + 20, uiRect.Top + 310);

            if (CurrentTarget != default(Vector2))
                DebugFont.DrawText($"当前目标: {CurrentTarget.X:F1}, {CurrentTarget.Y:F1}", uiRect.Left + 20, uiRect.Top + 330);
            else if (CurrentTarget_Monster != null)
                DebugFont.DrawText($"当前目标: 怪物 ({CurrentTarget_Monster.FloorCoordinate.X:F1}, {CurrentTarget_Monster.FloorCoordinate.Y:F1})", uiRect.Left + 20, uiRect.Top + 330);
            else if (CurrentTarget_ShrineOrPylon != null)
                DebugFont.DrawText($"当前目标: 神龛/塔 ({CurrentTarget_ShrineOrPylon.FloorCoordinate.X:F1}, {CurrentTarget_ShrineOrPylon.FloorCoordinate.Y:F1})", uiRect.Left + 20, uiRect.Top + 330);

            // 检查 PathPoints 是否为 null，避免空引用异常
            if (PathPoints != null)
                DebugFont.DrawText($"路径点数量: {PathPoints.Count}", uiRect.Left + 20, uiRect.Top + 350);
            else
                DebugFont.DrawText("路径点数量: 0", uiRect.Left + 20, uiRect.Top + 350);
                
            DebugFont.DrawText($"卡住次数: {StuckCount}/{MaxStuckCount}", uiRect.Left + 20, uiRect.Top + 370);

            for (int i = 0; i < DebugMessages.Count; i++)
            {
                DebugFont.DrawText(DebugMessages[i], uiRect.Left + 20, uiRect.Top + 390 + i * 20);
            }
            ...
            // 绘制边界点
            if (ShowPath && IsFollowingBoundary && BoundaryPoints != null && BoundaryPoints.Count > 0)
            {
                var boundaryBrush = Hud.Render.CreateBrush(200, 255, 165, 0, 2);
                var boundaryDecorator = new WorldDecoratorCollection(
                    new MapShapeDecorator(Hud)
                    {
                        ShapePainter = new CircleShapePainter(Hud),
                        Brush = boundaryBrush,
                ...
                {
                    var coord = Hud.Window.CreateWorldCoordinate(BoundaryPoints[i].X, BoundaryPoints[i].Y, Hud.Game.Me.FloorCoordinate.Z);
                    ((IWorldDecorator)boundaryDecorator).Paint(null, coord, null);
                }
            }
        }
        ...
        public void MoveToTarget()
        {
            ...
            // 检查战斗位移
            if (CurrentTarget_Monster != null && CombatStartPosition != default(Vector2))
            {
                if (CombatPositionCheckTimer.ElapsedMilliseconds > CombatPositionCheckInterval)
                {
                    float displacementDistance = Vector2.Distance(playerPos, CombatStartPosition);
                    if (displacementDistance > MaxCombatDisplacement)
                    {
                        AddDebugMessage($"战斗位移过大: {displacementDistance:F1}，重新计算路径");
                        UpdatePath();
                    }
                    CombatPositionCheckTimer.Restart();
                }
            }

            // 检查是否卡住
            if (LastPosition != default(Vector2) && Vector2.Distance(playerPos, LastPosition) < StuckThreshold)
            {
                if (StuckTimer.ElapsedMilliseconds > StuckTimeout)
                {
                    ...
                    // 检查是否在同一位置反复卡住
                    if (LastStuckPosition != default(Vector2) && Vector2.Distance(playerPos, LastStuckPosition) < 10.0f)
                    {
                        StuckCount++;
                        AddDebugMessage($"在相同位置卡住 {StuckCount} 次");
                        
                        // 如果在同一位置卡住多次，启用边界跟随算法
                        if (StuckCount >= MaxStuckCount)
                        {
                            IsFollowingBoundary = true;
                            BoundaryFollowTimer.Restart();
                            AddDebugMessage("多次卡住，启用边界跟随算法");
                        }
                    }
                    else
                    {
                        LastStuckPosition = playerPos;
                        StuckCount = 1;
                    }
            ...
            {
                // 检查是否超时
                if (BoundaryFollowTimer.ElapsedMilliseconds > BoundaryFollowTimeout)
                {
                    AddDebugMessage("边界跟随超时，切换回正常探索");
                    IsFollowingBoundary = false;
            ...
            if (nearbyMonsters.Any())
            {
                ...
                if (distanceToMonster < 15.0f)
                {
                    ...
                    // 设置战斗开始位置
                    if (CombatStartPosition == default(Vector2))
                    {
                        CombatStartPosition = playerPos;
                        CombatPositionCheckTimer.Restart();
                    }

                    // 如果在攻击范围内，施放技能攻击怪物
                    if (distanceToMonster < 10.0f)
                    {
                        ...
                        try
                        {
                            ...
                            if (screenCoord.X >= 0 && screenCoord.X <= Hud.Window.Size.Width && screenCoord.Y >= 0 && screenCoord.Y <= Hud.Window.Size.Height)
                            {
                                // 移动鼠标到怪物位置
        ...
        // 边界跟随函数
        private void FollowBoundary(Vector2 playerPos)
        {
            AddDebugMessage("执行边界跟随");
            
            // 检测周围8个方向是否有障碍物
            bool[] obstacles = new bool[8];
            float[] directions = new float[] { 0, 45, 90, 135, 180, 225, 270, 315 }; // 角度
            float detectionDistance = 15.0f;
            
            for (int i = 0; i < 8; i++)
            {
                float angle = directions[i] * (float)Math.PI / 180;
                float x = playerPos.X + detectionDistance * (float)Math.Cos(angle);
                float y = playerPos.Y + detectionDistance * (float)Math.Sin(angle);
                Vector2 checkPoint = new Vector2(x, y);
                
                obstacles[i] = !IsWalkable(checkPoint);
                AddDebugMessage($"方向 {directions[i]}°: {(obstacles[i] ? "障碍物" : "可通行")}");
            }
            
            // 判断当前是否沿着边界
            bool isNearBoundary = obstacles.Any(o => o);
            
            if (!isNearBoundary)
            {
                // 如果不在边界附近，寻找最近的边界
                AddDebugMessage("不在边界附近，寻找边界");
                
                // 使用螺旋搜索方式寻找边界
                float searchRadius = 20.0f;
                float searchAngle = 0;
                float angleStep = 30.0f * (float)Math.PI / 180; // 30度
                float radiusStep = 10.0f;
                bool foundBoundary = false;
                
                for (int i = 0; i < 36 && !foundBoundary; i++) // 最多搜索36次
                {
                    float x = playerPos.X + searchRadius * (float)Math.Cos(searchAngle);
                    float y = playerPos.Y + searchRadius * (float)Math.Sin(searchAngle);
                    Vector2 searchPoint = new Vector2(x, y);
                    
                    // 检查搜索点周围是否有障碍物
                    bool hasObstacle = false;
                    for (int j = 0; j < 8 && !hasObstacle; j++)
                    {
                        float checkAngle = directions[j] * (float)Math.PI / 180;
                        float checkX = searchPoint.X + detectionDistance * (float)Math.Cos(checkAngle);
                        float checkY = searchPoint.Y + detectionDistance * (float)Math.Sin(checkAngle);
                        Vector2 obstacleCheckPoint = new Vector2(checkX, checkY);
                        
                        if (!IsWalkable(obstacleCheckPoint))
                        {
                            hasObstacle = true;
                        }
                    }
                    ...
                    {
                        // 找到边界，移动到该点
                        AddDebugMessage($"找到边界点: ({searchPoint.X:F1}, {searchPoint.Y:F1})");
                        MoveToPosition(searchPoint);
                        foundBoundary = true;
                        
                        // 记录边界点
                        if (!VisitedBoundaryPoints.Contains(NormalizePoint(searchPoint)))
                        {
                            BoundaryPoints.Add(searchPoint);
                            VisitedBoundaryPoints.Add(NormalizePoint(searchPoint));
                        }
                    }
                    
                    // 更新搜索角度和半径
                    searchAngle += angleStep;
                    if (searchAngle >= 2 * Math.PI)
                    {
                        searchAngle = 0;
            ...
            // 如果在边界附近，记录当前边界点
            if (!VisitedBoundaryPoints.Contains(NormalizePoint(playerPos)))
            {
                BoundaryPoints.Add(playerPos);
                VisitedBoundaryPoints.Add(NormalizePoint(playerPos));
            }
            
            // 选择沿着边界移动的方向
            // 假定墙在我们的左手边，我们沿着墙前进（左手法则）
            int currentDirIndex = -1;
            for (int i = 0; i < 8; i++)
            {
                if (directions[i] == BoundaryFollowDirection.X && directions[i] == BoundaryFollowDirection.Y)
                {
                    currentDirIndex = i;
                    break;
                }
            ...
            // 寻找一个可走的方向，优先选择左手边靠墙的方向
            int nextDir = (currentDirIndex + 6) % 8; // 左手边的方向
            bool foundPath = false;
            
            for (int i = 0; i < 8; i++)
            {
                int checkDir = (nextDir + i) % 8;
                if (!obstacles[checkDir])
                {
                    // 找到可行方向
                    float angle = directions[checkDir] * (float)Math.PI / 180;
                    float moveX = playerPos.X + 20.0f * (float)Math.Cos(angle);
                    float moveY = playerPos.Y + 20.0f * (float)Math.Sin(angle);
                    Vector2 moveTarget = new Vector2(moveX, moveY);
                    
                    // 更新边界跟随方向
                    BoundaryFollowDirection = new Vector2((float)Math.Cos(angle), (float)Math.Sin(angle));
                    
                    AddDebugMessage($"沿边界移动到: ({moveTarget.X:F1}, {moveTarget.Y:F1}), 方向: {directions[checkDir]}°");
            ...
            if (!foundPath)
            {
                // 如果找不到可行方向，可能卡在死角，尝试后退
                AddDebugMessage("沿边界移动被卡住，尝试后退");
                float backAngle = (currentDirIndex + 4) % 8 * 45 * (float)Math.PI / 180; // 反方向
                float backX = playerPos.X + 20.0f * (float)Math.Cos(backAngle);
                float backY = playerPos.Y + 20.0f * (float)Math.Sin(backAngle);
                Vector2 backTarget = new Vector2(backX, backY);
                
                MoveToPosition(backTarget);
            }
        }
        ...
        private Vector2 NormalizePoint(Vector2 point)
        {
            // 将点四舍五入到最近的5单位网格点
            return new Vector2(
                (float)Math.Round(point.X / BoundaryPointInterval) * BoundaryPointInterval,
                (float)Math.Round(point.Y / BoundaryPointInterval) * BoundaryPointInterval
            );
        }
        ...
        private void CheckAndBreakObstacles(Vector2 playerPos)
        {
            ...
            var obstacles = Hud.Game.Actors
                .Where(a => (a.SnoActor.Kind == ActorKind.Chest || 
        ...
        public void UpdatePath()
        {
            try
            {
                ...
                if (IsFollowingBoundary)
                {
                    AddDebugMessage("边界跟随已启用，不再执行普通探索");
                    return;
                }
        ...
        private bool FindAndTargetObelisk()
        {
            ...
            // 需要排除的 Actor 名称
            string[] excludeNames = new string[] { "米瑞姆", "Miriam", "Myriam", "欧瑞克", "Orek", "卡德拉", "Kadala", "库尔", "Kulle" };
        ...
        private bool FindAndTargetShrine()
        {
            ...
            var shrine = Hud.Game.Actors.Where(a => 
                a.SnoActor.Kind == ActorKind.Shrine && 
                !a.IsOperated && 
                !a.IsDisabled)
        ...
        {
            var exit = Hud.Game.Actors.FirstOrDefault(a => 
                a.SnoActor.Kind == ActorKind.Portal && 
                a.SnoActor.Code.Contains("RiftPortal"));
        ...
        private void FindNewExplorationTarget()
        {
            try
            {
                ...
                // 1. 首先检查是否应该使用边界跟随算法
                if (StuckCount >= MaxStuckCount / 2)
                {
                    AddDebugMessage("卡住次数过多，切换到边界跟随算法");
                    IsFollowingBoundary = true;
                    BoundaryFollowTimer.Restart();
                    return;
                }
                ...
                // 3. 使用边界探索算法，沿着地图边界探索
                AddDebugMessage("使用边界探索算法");
                var mapBounds = GetMapBounds();
                var boundaryTarget = FindBoundaryExplorationTarget(playerPos, mapBounds);
                
                if (boundaryTarget != default(Vector2))
                {
                    CurrentTarget = boundaryTarget;
                    CurrentTarget_Monster = null;
                    CurrentTarget_ShrineOrPylon = null;
                    AddDebugMessage($"选择边界探索点作为新目标: ({CurrentTarget.X:F1}, {CurrentTarget.Y:F1})");
                    return;
                }

                // 4. 如果边界探索失败，尝试找未探索的区域
                ...
                // 5. 如果以上方法都失败，回到最初探索的区域（可能有漏掉的地方）
                ...
                // 如果所有方法都失败，切换到边界跟随算法
                AddDebugMessage("所有探索方法都失败，切换到边界跟随算法");
                IsFollowingBoundary = true;
                BoundaryFollowTimer.Restart();
            }
            catch (Exception ex)
        ...
        // 基于边界跟随算法找目标
        private Vector2 FindBoundaryExplorationTarget(Vector2 playerPos, (float MinX, float MaxX, float MinY, float MaxY) mapBounds)
        {
            try
            {
                // 地图四条边界的距离
                float distToLeft = playerPos.X - mapBounds.MinX;
                float distToRight = mapBounds.MaxX - playerPos.X;
                float distToTop = playerPos.Y - mapBounds.MinY;
                float distToBottom = mapBounds.MaxY - playerPos.Y;
                
                // 找出最近的边界
                float minDist = Math.Min(Math.Min(distToLeft, distToRight), Math.Min(distToTop, distToBottom));
                
                // 选择沿着最近边界方向探索的点
                Vector2 targetPoint = default(Vector2);
                float exploreDistance = 100.0f; // 探索距离
                
                if (minDist == distToLeft)
                {
                    // 靠近左边界，向上或向下移动
                    AddDebugMessage("靠近左边界，选择向上或向下移动");
                    float y = playerPos.Y + (new Random().Next(2) == 0 ? -1 : 1) * exploreDistance;
                    y = Math.Max(mapBounds.MinY + 50, Math.Min(y, mapBounds.MaxY - 50)); // 确保在地图范围内
                    targetPoint = new Vector2(playerPos.X + 20.0f, y); // 稍微向右移动一点，避免卡在边界
                }
                else if (minDist == distToRight)
                {
                    // 靠近右边界，向上或向下移动
                    AddDebugMessage("靠近右边界，选择向上或向下移动");
                    float y = playerPos.Y + (new Random().Next(2) == 0 ? -1 : 1) * exploreDistance;
                    y = Math.Max(mapBounds.MinY + 50, Math.Min(y, mapBounds.MaxY - 50));
                    targetPoint = new Vector2(playerPos.X - 20.0f, y); // 稍微向左移动一点
                }
                else if (minDist == distToTop)
                {
                    // 靠近上边界，向左或向右移动
                    AddDebugMessage("靠近上边界，选择向左或向右移动");
                    float x = playerPos.X + (new Random().Next(2) == 0 ? -1 : 1) * exploreDistance;
                    x = Math.Max(mapBounds.MinX + 50, Math.Min(x, mapBounds.MaxX - 50));
                    targetPoint = new Vector2(x, playerPos.Y + 20.0f); // 稍微向下移动一点
                }
                else // distToBottom
                {
                    // 靠近下边界，向左或向右移动
                    AddDebugMessage("靠近下边界，选择向左或向右移动");
                    float x = playerPos.X + (new Random().Next(2) == 0 ? -1 : 1) * exploreDistance;
                    x = Math.Max(mapBounds.MinX + 50, Math.Min(x, mapBounds.MaxX - 50));
                    targetPoint = new Vector2(x, playerPos.Y - 20.0f); // 稍微向上移动一点
                }
                
                // 检查目标点是否已探索
                if (ExploredPoints.Any(p => Vector2.Distance(p, targetPoint) < 30.0f))
                {
                    // 如果目标已探索，尝试另一个方向
                    AddDebugMessage("目标点已探索，尝试另一个方向");
                    
                    // 选择一个不同的方向
                    float angle = (float)new Random().NextDouble() * 2 * (float)Math.PI; // 随机角度
                    targetPoint = new Vector2(
                        playerPos.X + exploreDistance * (float)Math.Cos(angle),
                        playerPos.Y + exploreDistance * (float)Math.Sin(angle)
                    );
                    
                    // 确保在地图范围内
                    targetPoint.X = Math.Max(mapBounds.MinX + 50, Math.Min(targetPoint.X, mapBounds.MaxX - 50));
                    targetPoint.Y = Math.Max(mapBounds.MinY + 50, Math.Min(targetPoint.Y, mapBounds.MaxY - 50));
                }
            ...
            {
                AddDebugMessage($"寻找边界探索目标出错: {ex.Message}");
        ...
        private (float MinX, float MaxX, float MinY, float MaxY) GetMapBounds()
        {
            try
            {
                ...
                if (!actors.Any()) return (float.MinValue, float.MaxValue, float.MinValue, float.MaxValue);
        ...
        private void HandleStuck(Vector2 playerPos)
        {
            ...
            // 标记周围区域为已探索，避免再次尝试
            float avoidRadius = 15.0f; // 增加避免区域半径
            ...
            // 如果边界跟随已启用，尝试沿另一个方向移动
            if (IsFollowingBoundary)
            {
                AddDebugMessage("边界跟随中卡住，尝试改变方向");
                
                // 尝试旋转90度
                float angle = (float)Math.Atan2(BoundaryFollowDirection.Y, BoundaryFollowDirection.X);
                angle += (float)Math.PI / 2; // 旋转90度
                BoundaryFollowDirection = new Vector2((float)Math.Cos(angle), (float)Math.Sin(angle));
                
                // 沿新方向前进
                float moveDistance = 30.0f;
                Vector2 newTarget = new Vector2(
                    playerPos.X + BoundaryFollowDirection.X * moveDistance,
                    playerPos.Y + BoundaryFollowDirection.Y * moveDistance
                );
                
                AddDebugMessage($"边界跟随新方向: ({BoundaryFollowDirection.X:F2}, {BoundaryFollowDirection.Y:F2})");
            ...
            if (!escapeSuccessful)
            {
                ...
                {
                    // 尝试三个不同的距离
                    float[] distances = new float[] { 30.0f, 50.0f, 70.0f }; // 增加距离尝试更远的逃脱
            ...
            if (!escapeSuccessful)
            {
                AddDebugMessage("所有常规方法都失败，启用边界跟随算法");
                IsFollowingBoundary = true;
                BoundaryFollowTimer.Restart();
